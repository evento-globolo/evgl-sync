...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let app = Router::new()
        .route(
            "/healthz",
            get(|| async { Json(serde_json::json!({"status":"ok","service":"evgl-sync"})) }),
        )
    ...
    info!(%addr, "Evento Globolo sync gateway listening");
...
async fn reconcile(
    ...
    Ok(Json(ReconcileResponse {
        ...
        contract: "evgl_interfaces::Event",
    }))
}
...
mod tests {
    ...
    #[test]
    fn merges_records_by_id_and_accepts_newer_state() {
        let base = serde_json::json!([
            {"id":"event-1","updated_at":"2026-08-04T10:00:00Z","title":"Old"}
        ]);
        let incoming = serde_json::json!([
            {"id":"event-1","updated_at":"2026-08-04T11:00:00Z","title":"New"},
            {"id":"event-2","updated_at":"2026-08-04T11:00:00Z","title":"Second"}
        ]);
        let merged = reconcile_values(&base, &incoming, &merge_options()).unwrap();
        assert_eq!(merged[0]["title"], "New");
        assert_eq!(merged[1]["id"], "event-2");
    }

    #[test]
    fn rejects_older_last_writer_state() {
        let base = serde_json::json!([
            {"id":"event-1","updated_at":"2026-08-04T11:00:00Z","title":"Current"}
        ]);
        let incoming = serde_json::json!([
            {"id":"event-1","updated_at":"2026-08-04T10:00:00Z","title":"Stale"}
        ]);
        let merged = reconcile_values(&base, &incoming, &merge_options()).unwrap();
        assert_eq!(merged[0]["title"], "Current");
    }
}
