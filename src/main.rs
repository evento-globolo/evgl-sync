...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let app = Router::new()
        .route("/healthz", axum::routing::get(|| async { Json(serde_json::json!({"status":"ok","service":"evgl-sync"})) }))
...
async fn reconcile(
    ...
    Ok(Json(ReconcileResponse { merged, engine: "opto-sync/syncer.rs", contract: "evgl_interfaces::Event" }))
}
